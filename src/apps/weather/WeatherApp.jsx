...
// given: full current-conditions card with humidity + wind on the page, and a
// compact emoji + temperature glance in a small (~300px) widget, reflowing live
  ...
  return (
    <div className="@container flex h-full min-h-0 flex-col gap-4">
      <div className="flex shrink-0 gap-2">
        ...
        />
        <Button onClick={search} disabled={showOffline || status === 'loading'} aria-label="Search">
          {status === 'loading' ? <Loader2 className="animate-spin" /> : <Search />}
          <span className="hidden @sm:inline">Search</span>
        </Button>
      </div>

      <div className="flex min-h-0 flex-1 flex-col items-center justify-center gap-4 overflow-auto @sm:justify-start">
        {showOffline ? (
          <OfflineCard />
        ) : (
          <>
            ...
            {status === 'idle' && !data && (
              <p className="text-center text-sm text-muted-foreground">Search a city.</p>
            )}
            {data && (
              <Card className="w-full">
                <CardContent className="flex flex-col items-center gap-3 text-center">
                  ...
                  <div className="flex items-center gap-3 @sm:gap-4">
                    <span className="text-4xl @sm:text-5xl" aria-hidden="true">{emoji}</span>
