...
// Countdown timers feature. The full H:M:S create form appears when there's
// room (@sm+); the widget shows presets + the live list only.
function Timers() {
    ...
    <div className="space-y-4">
      <div className="rounded-lg border p-3 @sm:p-4">
        ...
        <div className="flex flex-wrap gap-2 @sm:mt-3">
          {PRESETS.map((p) => (
            <Button key={p.label} variant="outline" size="sm" className="flex-1 @sm:flex-none" onClick={() => add(p.ms, '')}>
              {p.label}
            </Button>
          ))}
        </div>
      ...
      {timers.length === 0 ? (
        <div className="flex flex-col items-center gap-2 rounded-lg border border-dashed py-10 text-center text-muted-foreground">
          ...
          <p className="text-sm">No timers running — pick a preset above.</p>
        </div>
      ) : (
        <ul className="space-y-2">
          {timers.map((t) => {
            ...
            return (
              <li key={t.id} className="rounded-lg border px-3 py-2.5 @sm:px-4 @sm:py-3">
                <div className="flex items-center justify-between gap-3">
                  <div className="min-w-0">
                    <div className={cn('font-mono text-2xl font-semibold tabular-nums @sm:text-3xl', t.done && 'text-primary')}>
                      {fmtDuration(left)}
                    </div>
                  ...
                  <div className="flex items-center gap-1">
                    {t.done ? (
                      <Button variant="outline" size="icon" className="size-8 @sm:size-9" onClick={() => restart(t.id)} aria-label="Restart">
                        <RotateCcw className="size-4" />
                      </Button>
                    ) : t.running ? (
                      <Button variant="secondary" size="icon" className="size-8 @sm:size-9" onClick={() => pause(t.id)} aria-label="Pause">
                        <Pause className="size-4" />
                      </Button>
                    ) : (
                      <Button size="icon" className="size-8 @sm:size-9" onClick={() => resume(t.id)} aria-label="Resume">
                        <Play className="size-4" />
                      </Button>
                    )}
                    <Button variant="ghost" size="icon" className="size-8 @sm:size-9" onClick={() => remove(t.id)} aria-label="Delete">
