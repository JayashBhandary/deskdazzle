...
function TaskRow({ item, project, subCount, onToggle, onDelete, onOpen }) {
  const { todo } = item;
  // Metadata (project, due, recurrence, priority, tags, subtask count) stacks
  // BELOW the title rather than sharing its line — so a narrow widget keeps the
  // title fully visible and the badges just wrap underneath.
  const hasMeta =
  ...
  return (
    <div className="group flex items-start gap-3 rounded-md border bg-card px-3 py-2">
      <Checkbox
