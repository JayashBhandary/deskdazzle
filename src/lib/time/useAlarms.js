...
// a repeating one stays armed for the next day. Firing is edge-triggered on the
// minute boundary, so opening the app partway through an alarm's minute won't
// retro-fire it.
//
// An alarm: { id, time: "HH:MM", label, enabled, repeat }
...
export function useAlarmsEngine() {
  ...
  useEffect(() => {
    const check = () => {
      ...
      const due = ref.current.filter((a) => a.enabled && a.time === now);
      ...
      playChime(4);
      // One-shot alarms switch off after ringing.
      setAlarms((list) => list.map((a) => (due.includes(a) && !a.repeat ? { ...a, enabled: false } : a)));
    };
    const id = setInterval(check, 1000);
    return () => clearInterval(id);
  }, [setAlarms]);

  const add = useCallback((time, label = '', repeat = false) => {
    if (!time) return;
    requestNotifyPermission();
    setAlarms((list) => [...list, { id: uid(), time, label, enabled: true, repeat }]);
  }, [setAlarms]);
